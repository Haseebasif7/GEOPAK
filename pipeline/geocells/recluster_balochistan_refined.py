"""Refined re-clustering for Balochistan to reach target."""
...
HDBSCAN_KWARGS = dict(
    ...
    min_samples=5,  # Lowered from 6 to 5 (20% reduction from original 10)
    metric="euclidean",
)

# Target cells for Balochistan
TARGET_CELLS = 50  # Target 48-55, using 50 as middle
MAX_RADIUS_M = 60_000.0  # 60 km target (slightly above 55-65 avg target)
...
def _best_effort_radius_reduction(df: pd.DataFrame) -> pd.DataFrame:
    ...
    max_iterations = 300  # Increased for more aggressive reduction
    ...
    for iteration in range(max_iterations):
        ...
        if not success:
            # Try next worst cluster
            sorted_radii = sorted(radii.items(), key=lambda kv: kv[1], reverse=True)
            found_split = False
            for cid, radius in sorted_radii[1:]:  # Skip the worst one we just tried
                if radius <= MAX_RADIUS_M:
                    break
                success, new_df, next_cluster_id = _split_cluster_if_feasible(
                    df, cid, next_cluster_id, MIN_SAMPLES_PER_CELL
                )
                if success:
                    found_split = True
                    break
...
def main():
    print("REFINED RE-CLUSTERING FOR BALOCHISTAN")
    ...
    # Step 2: HDBSCAN clustering with refined parameters
    # Step 2: HDBSCAN clustering with refined parameters
    ...
    # Step 4: Aggressive radius reduction
    print(f"Step 4: Reducing radius (target: {MAX_RADIUS_M/1000:.0f} km max)...")
    gproj = _best_effort_radius_reduction(gproj)
    
    # Step 5: Match target cell count
    print(f"Step 5: Matching target cell count ({TARGET_CELLS})...")
    n_samples = len(gproj)
